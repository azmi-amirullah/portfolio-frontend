...
import { MdSearch, MdReceipt, MdSync } from 'react-icons/md';
...
const DATE_FILTER_OPTIONS = [
  { value: 'recent', label: 'Recent (Last 10)' },
...
export default function SalesHistoryPage() {
  ...
  const calculateProfit = useCallback((sale: Transaction) => {
    const cost = sale.products.reduce(
      (sum, p) => sum + (p.buyPrice || 0) * p.quantity,
      0
    );
    ...
    <div className='space-y-6 md:pb-0'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        <div className='bg-white rounded-2xl p-6 border border-gray-200 shadow-sm'>
          <div className='text-gray-500 font-medium mb-1'>Transaction</div>
          <div className='text-2xl md:text-3xl font-bold'>
            {totalTransactions}
          </div>
        </div>
        <div className='bg-white rounded-2xl p-6 border border-gray-200 shadow-sm'>
          <div className='text-gray-500 font-medium mb-1'>Sold</div>
          <div className='text-2xl md:text-3xl font-bold'>{totalItemsSold}</div>
        </div>
        <div className='bg-blue-600 rounded-2xl p-6 text-white shadow-lg shadow-blue-200'>
          <div className='text-white/80 font-medium mb-1'>Revenue</div>
          <div className='text-2xl md:text-3xl font-bold'>
      ...
      <div className='flex flex-col md:flex-row gap-4'>
        ...
        <div className='flex gap-3'>
          <div className='flex-1 md:flex-none md:min-w-[200px]'>
            <Select
              value={DATE_FILTER_OPTIONS.find(
          ...
          {filteredSales.map((sale) => (
            ...
            >
              <div className='flex justify-between items-start'>
                <div>
                  <div className='font-mono text-gray-500 mb-1'>{sale.id}</div>
                  <div className='font-bold '>{formatDate(sale.timestamp)}</div>
