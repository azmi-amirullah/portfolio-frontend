    ...
    <>
      <nav
        className={`fixed w-full z-50 ${
          mounted ? 'opacity-100 transition-all duration-300' : 'opacity-0'
        } ${
          scrolled
      ...
      {isOpen && (
        <div
          id='mobile-menu'
          className='md:hidden fixed left-0 w-full top-16 bg-white border-b border-border px-6 py-2 flex flex-col shadow-lg z-40 animate-in slide-in-from-top-5'
        >
          {navLinks.map((link) => (
            <Button
              key={link.name}
